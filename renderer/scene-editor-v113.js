...
(() => {
  ...
  window.addEventListener('keydown',event=>{if(event.code==='Escape'&&local.lua119.pointPicker){event.preventDefault();cancelAbilityPoint125();return;}if(event.code==='Space'&&UI.activeModuleId==='combat'&&!event.repeat){local.spaceDown=true;document.getElementById('combat-stage-viewport')?.classList.add('space-pan-v105');event.preventDefault();}});
