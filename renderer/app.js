...
/* v49 auto updater ui */
(function(){
  const updaterState = { last: null, unsubscribe: null, bound: false };

  function updaterStatusText(status = {}) {
    if (!window.electronAPI?.getUpdateStatus) return 'Автообновления недоступны в этой сборке';
    const current = status.status || 'idle';
    if (current === 'checking') return status.message || 'Проверка обновлений...';
    if (current === 'available') return `Доступна версия ${status.version || ''}`.trim();
    if (current === 'downloading') return status.message || `Скачивание: ${Math.round(Number(status.percent || 0))}%`;
    if (current === 'downloaded') return status.message || 'Обновление загружено';
    if (current === 'none') return 'Обновлений нет';
    if (current === 'dev') return status.message || 'Updater работает только в собранном приложении';
    if (current === 'unavailable') return status.message || 'electron-updater не установлен';
    if (current === 'error') return `Ошибка обновления: ${status.message || 'unknown error'}`;
    if (current === 'ready') return status.message || 'Updater готов';
    return status.message || 'Готов к проверке обновлений';
  }
  ...
  function renderUpdaterPanel() {
    ...
    if (!panel) {
      ...
      panel.className = 'card pad18 updater-profile-panel';
      panel.innerHTML = `
        <div class="section-title">Обновления приложения</div>
        <div class="small-note updater-status-text" style="margin-bottom:12px">Готов к проверке обновлений</div>
        <div class="row" style="justify-content:flex-start;gap:10px;flex-wrap:wrap">
          <button class="secondary" type="button" data-updater-action="check">ПРОВЕРИТЬ</button>
          <button class="secondary" type="button" data-updater-action="download" disabled>СКАЧАТЬ</button>
          <button class="primary" type="button" data-updater-action="install" disabled>ПЕРЕЗАПУСТИТЬ И УСТАНОВИТЬ</button>
        </div>
        <div class="updater-progress-wrap" style="margin-top:12px;display:none"><div class="updater-progress-bar"></div></div>
      `;
      form.insertAdjacentElement('afterend', panel);
    }
    bindUpdaterPanel(panel);
    updateUpdaterPanel(updaterState.last || {});
  }

  function bindUpdaterPanel(panel) {
    if (panel.dataset.boundUpdaterPanel === '1') return;
    panel.dataset.boundUpdaterPanel = '1';
    panel.addEventListener('click', async event => {
      const button = event.target.closest('[data-updater-action]');
      if (!button) return;
      const action = button.dataset.updaterAction;
      button.disabled = true;
      try {
        let res = null;
        if (action === 'check') res = await window.electronAPI?.checkForUpdates?.();
        if (action === 'download') res = await window.electronAPI?.downloadUpdate?.();
        if (action === 'install') res = await window.electronAPI?.installUpdate?.();
        if (res) updateUpdaterPanel(res);
      } catch (error) {
        updateUpdaterPanel({ status: 'error', message: error?.message || String(error) });
      } finally {
        const status = updaterState.last || {};
        button.disabled = action === 'download' ? status.status !== 'available' : action === 'install' ? status.status !== 'downloaded' : false;
      }
    });
  }

  function updateUpdaterPanel(status = {}) {
    updaterState.last = { ...(updaterState.last || {}), ...status };
    const panel = document.getElementById('updater-profile-panel');
    if (!panel) return;
    const text = panel.querySelector('.updater-status-text');
    if (text) text.textContent = updaterStatusText(updaterState.last);
    const checkBtn = panel.querySelector('[data-updater-action="check"]');
    const downloadBtn = panel.querySelector('[data-updater-action="download"]');
    const installBtn = panel.querySelector('[data-updater-action="install"]');
    if (checkBtn) checkBtn.disabled = updaterState.last.status === 'checking' || updaterState.last.status === 'downloading';
    if (downloadBtn) downloadBtn.disabled = updaterState.last.status !== 'available';
    if (installBtn) installBtn.disabled = updaterState.last.status !== 'downloaded';
    const wrap = panel.querySelector('.updater-progress-wrap');
    const bar = panel.querySelector('.updater-progress-bar');
    const isDownloading = updaterState.last.status === 'downloading';
    if (wrap) wrap.style.display = isDownloading ? 'block' : 'none';
    if (bar) bar.style.width = `${Math.max(0, Math.min(100, Number(updaterState.last.percent || 0)))}%`;
  }

  function bootstrapUpdaterUi() {
    if (updaterState.bound) return;
    updaterState.bound = true;
    if (window.electronAPI?.onUpdaterStatus) {
      updaterState.unsubscribe = window.electronAPI.onUpdaterStatus(status => {
        updateUpdaterPanel(status || {});
      });
    }
    window.electronAPI?.getUpdateStatus?.().then(status => updateUpdaterPanel(status || {})).catch(() => {});
  }

  const __renderProfileUpdaterV49 = UI.renderProfile.bind(UI);
  UI.renderProfile = function() {
    __renderProfileUpdaterV49();
    bootstrapUpdaterUi();
    renderUpdaterPanel();
  };
