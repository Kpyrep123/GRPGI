  ...
  async function importAsset104(){
    try{
      ...
      if(window.electronAPI?.chooseCombatMedia&&window.electronAPI?.saveLocalCombatAssetFile){
        ...
        const stem=(filePath.split(/[\/]/).pop()||'scene_library').replace(/\.[^.]+$/,'');
      ...
      const fallbackName=((saved.file||'').split(/[\/]/).pop()||'Новый ассет').replace(/\.[^.]+$/,'');
      const name=String(prompt('Название ассета',fallbackName)||fallbackName).trim()||'Новый ассет';
