/* v1.0.107 — tactical Scene Editor */
(() => {
  'use strict';
  if (window.__grpgSceneEditorV107 || typeof Combat === 'undefined') return;
  window.__grpgSceneEditorV107 = true;
  ...
  function tokenVisionCells(token, scene) {
    ...
    if (token?.equipmentItemId && ['turret','drone'].includes(String(token.equipmentType || equipmentItem105(token.equipmentItemId)?.type || ''))) {
      return Math.max(0, num(equipmentItem105(token.equipmentItemId)?.unitVisionRange, 0) / scalePerHex(scene));
    }
  ...
  function tokenMoveCells(token, scene) {
    ...
    if (token?.equipmentItemId && ['turret','drone'].includes(String(token.equipmentType || equipmentItem105(token.equipmentItemId)?.type || ''))) {
      return Math.max(0, num(equipmentItem105(token.equipmentItemId)?.unitMoveRange, 0) / scalePerHex(scene));
    }
    const wc = combatConfig(entityForToken(token) || {});
  ...
  function tokenAttackCells(token, scene) {
    ...
    const candidate = num(weapon.range ?? weapon.attackRange ?? weapon.maxRange ?? weapon.distance, 0);
    return Math.max(0, candidate / scalePerHex(scene));
  }
  function grenadeRangeCells105(item,scene){return Math.max(0,num(item?.grenadeRange??item?.throwRange??item?.range,0))/scalePerHex(scene);}
  function grenadeRadiusCells105(item,scene){return Math.max(0,num(item?.grenadeRadius??item?.blastRadius??item?.radius,0))/scalePerHex(scene);}
  ...
  function nearestHexCenter107(scene, point={x:0,y:0}){
    const w=Math.max(1,num(scene?.width,20)),h=Math.max(1,num(scene?.height,12));
    const {side,hexH,stepX,tokenSize}=hexMetrics107();
    const approx=Math.round((num(point.x)-side)/stepX);
    let best={x:side,y:hexH/2},bestD=Infinity;
    for(let col=Math.max(0,approx-2);col<=approx+2;col++){
      const cx=side+col*stepX;if(cx<tokenSize/2||cx>w-tokenSize/2)continue;
      const offset=(col%2)*hexH/2,base=hexH/2+offset;
      const row0=Math.round((num(point.y)-base)/hexH);
      for(let row=Math.max(0,row0-2);row<=row0+2;row++){
        const cy=base+row*hexH;if(cy<tokenSize/2||cy>h-tokenSize/2)continue;
        const d=(cx-num(point.x))**2+(cy-num(point.y))**2;
  ...
  // v1.0.106: camera transform must survive UI rerenders exactly. Clamping is done only while the user pans/zooms.
  Combat.getViewTransform=function(sceneId=this.getSceneIdForView?.(),viewportEl=document.getElementById('combat-stage-viewport')){
    const view=this.getViewState?.(sceneId)||{zoom:1,panX:0,panY:0};
    const zoom=clamp104(num(view.zoom,1),.45,3.5),panX=num(view.panX,0),panY=num(view.panY,0);
    return `translate(${panX.toFixed(1)}px, ${panY.toFixed(1)}px) scale(${zoom.toFixed(3)})`;
  };
  ...
  function renderActionEquipment105(scene,runtime){
    ...
    if(local.ui.action==='attack'){
      if(token.equipmentItemId&&['turret','drone'].includes(token.equipmentType)){const w=weaponForToken105(token);return `<span class="scene-action-equipment-v105"><b>${html(w?.name||token.name)}</b><small>${html(w?.damage||'—')} · ${num(w?.range ?? token.attackRange ?? DEFAULT_ATTACK_WORLD)} ${html(scene.scaleLabel)}</small></span>`;}
      ...
      return `<label class="scene-action-select-v105">ОРУЖИЕ <select class="select" id="scene-weapon-v105">${weapons.map(w=>`<option value="${html(w.id)}" ${w.id===token.weaponId?'selected':''}>${html(w.name)} · ${html(w.damage||'—')} · ${num(w.range ?? DEFAULT_ATTACK_WORLD)} ${html(scene.scaleLabel)}</option>`).join('')}</select></label>`;
    }
  ...
  function renderTokenEquipmentInspector105(token,scene){
    if(token.equipmentItemId&&['turret','drone'].includes(token.equipmentType)){const item=weaponForToken105(token);return `<div class="scene-token-loadout-v105"><b>Встроенная атака</b><span>${html(item?.name||token.name)} · ${html(item?.damage||'—')} · дальность ${num(item?.range ?? DEFAULT_ATTACK_WORLD)} ${html(scene.scaleLabel)}</span></div>`;}
  ...
  function renderInspector104(scene,runtime){
    ...
    if(selected.kind==='token'){
      ...
      return `<aside class="scene-inspector-v104"><div class="scene-inspector-head-v104"><b>ЮНИТ</b><span>${html(o.name)}</span></div><form class="scene-inspector-form-v104" data-object-form-v104="token">
        ...
        <div class="field"><label>Переопределение дальности атаки</label><input class="input" type="number" min="0" step="0.1" name="attackRange" value="${num(o.attackRange,0)}"/><div class="small-note">0 = дальность выбранного оружия / встроенной атаки.</div></div>
        ${renderTokenEquipmentInspector105(o,scene)}
        <div class="small-note">Эффективно: обзор ${(tokenVisionCells(o,scene)*scalePerHex(scene)).toFixed(1)} · движение ${(tokenMoveCells(o,scene)*scalePerHex(scene)).toFixed(1)} · атака ${(tokenAttackCells(o,scene)*scalePerHex(scene)).toFixed(1)} ${html(scene.scaleLabel)}</div>
  ...
  function makeRoutePreview104(point,attack=false){
    ...
    if(attack){const weapon=weaponForToken105(token);if(!weapon)return {kind:'route',mode:'attack',points:[start,start],end:start,limited:true,blocked:true,label:'Нет выбранного оружия'};const max=tokenAttackCells(token,scene),distance=dist(start,point),limited=distance>max,ratio=distance>1e-6?max/distance:0,end=limited?{x:start.x+(point.x-start.x)*ratio,y:start.y+(point.y-start.y)*ratio}:point,blocked=!limited&&sightSegmentBlocked105(scene,start,end,[token.id]);return {kind:'route',mode:'attack',points:[start,end],end,limited,blocked,label:`${weapon.name||'Выстрел'}: ${(Math.min(distance,max)*scalePerHex(scene)).toFixed(1)} / ${(max*scalePerHex(scene)).toFixed(1)} ${scene.scaleLabel}${blocked?' · ЛИНИЯ ОГНЯ БЛОКИРОВАНА':''}`};}
    const snappedTarget=nearestHexCenter107(scene,point),path=findPath(scene,start,snappedTarget,token.id),totalMax=tokenMoveCells(token,scene),spentWorld=Math.max(0,num(token.movedThisTurn)),spentCells=spentWorld/scalePerHex(scene),max=Math.max(0,totalMax-spentCells);if(!path){return {kind:'route',mode:'move',points:[start],end:start,limited:true,blocked:true,label:'Путь заблокирован'};}const cut=truncatePath(path,max),usedWorld=Math.min(pathLength(path),max)*scalePerHex(scene),remainingWorld=Math.max(0,max*scalePerHex(scene));return {kind:'route',mode:'move',points:cut.points,end:cut.end,limited:cut.limited,label:`Движение: ${usedWorld.toFixed(1)} / осталось ${remainingWorld.toFixed(1)} ${scene.scaleLabel}`};
  }
  function makeGrenadePreview105(point){const scene=Combat.getScene(),token=selectedToken104(),grenade=grenadeForToken105(token);if(!scene||!token||!grenade)return null;const start=centerOf(token),max=grenadeRangeCells105(grenade,scene),distance=dist(start,point),limited=distance>max,ratio=distance>1e-6?max/distance:0,end=limited?{x:start.x+(point.x-start.x)*ratio,y:start.y+(point.y-start.y)*ratio}:point,radius=grenadeRadiusCells105(grenade,scene);return {kind:'grenade',mode:'grenade',start,end,radius,limited,itemId:grenade.id,label:`${grenade.name}: бросок ${(Math.min(distance,max)*scalePerHex(scene)).toFixed(1)} / ${(max*scalePerHex(scene)).toFixed(1)} · радиус ${(radius*scalePerHex(scene)).toFixed(1)} ${scene.scaleLabel}`};}
  function commitMove104(){const token=selectedToken104(),scene=Combat.getScene(),p=local.preview;if(!token||!scene||p?.kind!=='route'||p.mode!=='move'||p.blocked)return;if(!Combat.canMoveToken?.(token)){Toast.show('Сейчас не ход этого юнита','info');return;}fitTokenToHex107(token,scene,nearestHexCenter107(scene,p.end));token.movedThisTurn=num(token.movedThisTurn)+pathLength(p.points)*scalePerHex(scene);queueStoreSave();Combat.render();broadcastScene104();}
  ...
  async function persistCombatProfiles105(reason='Сцена: боевое действие'){try{await App.saveState?.(reason);}catch(error){console.error('SCENE_PROFILE_SAVE_FAILED',error);}}
  ...
  function commitAttack105(targetId) {
    ...
    const from = centerOf(attacker), to = centerOf(target), max = tokenAttackCells(attacker, scene), distance = dist(from, to);
    local.preview = makeRoutePreview104(to, true); refreshPreviewDom104();
    if (distance > max + .0001) { Toast.show(`Цель вне дальности: ${(distance * scalePerHex(scene)).toFixed(1)} / ${(max * scalePerHex(scene)).toFixed(1)} ${scene.scaleLabel}`, 'info'); return; }
  ...
  document.addEventListener('click',event=>{
    ...
    if(t.dataset.sceneActionV104){const action=t.dataset.sceneActionV104;local.ui.pendingPlacement=null;if(action==='clear'){local.preview=null;refreshPreviewDom104();return;}local.ui.action=action;Combat.render();return;}
