/* v1.0.108 — stable tactical Scene Editor */
(() => {
  'use strict';
  if (window.__grpgSceneEditorV108 || typeof Combat === 'undefined') return;
  window.__grpgSceneEditorV108 = true;
  ...
  function blankStore() {
    return { version: 2, cameraSchema: 108, scenes: {}, runtimes: {}, cameraByScene: {}, categories: ['Общее'], assetCategories: ['Общее'], assetLibrary: [], selectedSceneId: '', legacyImportedAt: null, updatedAt: null };
  }
  function normalizeStore(raw = {}) {
    ...
    next.cameraByScene = cameraSchema === 108 && next.cameraByScene && typeof next.cameraByScene === 'object' && !Array.isArray(next.cameraByScene) ? next.cameraByScene : {};
    next.cameraSchema = 108;
  ...
  function makeRoutePreview104(point,attack=false){
    ...
    if(!canMechanicalMove108(token))return {kind:'route',mode:'move',points:[start],end:start,steps:0,limited:true,blocked:true,label:'Выбран не текущий юнит инициативы'};const snappedTarget=nearestHexCenter107(scene,point),path=findHexPath108(scene,start,snappedTarget,token.id),totalMax=tokenMoveCells(token,scene),spentWorld=Math.max(0,num(token.movedThisTurn)),spentCells=spentWorld/scalePerHex(scene),max=Math.max(0,totalMax-spentCells);if(!path){return {kind:'route',mode:'move',points:[start],end:start,steps:0,limited:true,blocked:true,label:'Путь заблокирован'};}const cut=truncateHexPath108(path,max),usedWorld=cut.steps*scalePerHex(scene),remainingWorld=Math.max(0,(max-cut.steps)*scalePerHex(scene));return {kind:'route',mode:'move',points:cut.points,end:cut.end,steps:cut.steps,limited:cut.limited,label:`Движение: ${usedWorld.toFixed(1)} / после хода останется ${remainingWorld.toFixed(1)} ${scene.scaleLabel}`};
  }
  ...
  function commitMove104(){const token=selectedToken104(),scene=Combat.getScene(),p=local.preview;if(!token||!scene||p?.kind!=='route'||p.mode!=='move')return;if(p.blocked){Toast.show(p.label||'Путь заблокирован','info');return;}if(!canMechanicalMove108(token)){Toast.show('Сейчас не ход этого юнита','info');return;}if(num(p.steps,0)<=0)return;fitTokenToHex107(token,scene,p.end);token.movedThisTurn=num(token.movedThisTurn)+num(p.steps,0)*scalePerHex(scene);local.preview={...p,effect:'move',movedTokenId:token.id};queueStoreSave();Combat.render();broadcastScene104();setTimeout(()=>{if(local.preview?.effect==='move'&&local.preview?.movedTokenId===token.id){local.preview=null;refreshPreviewDom104();}},650);}
