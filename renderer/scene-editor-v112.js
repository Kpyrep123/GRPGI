...
(() => {
  ...
  function sightBlockedAt105(scene,x,y,ignoreIds=new Set()){
    const p={x,y};
    for(const asset of list(scene.assets)){if(!asset.blockSight)continue;if(pointInAssetAlpha110(asset,x,y,0))return true;}
  ...
  function applySceneForm104(form){const scene=Combat.getScene();if(!scene)return;const fd=new FormData(form);scene.name=String(fd.get('name')||scene.name).trim()||'Сцена';scene.category=String(fd.get('category')||'Общее').trim()||'Общее';scene.parentId=String(fd.get('parentId')||'');if(scene.parentId===scene.id||sceneDescendantIds(scene.id).has(scene.parentId))scene.parentId='';scene.width=clamp104(fd.get('width'),8,100);scene.height=clamp104(fd.get('height'),8,100);scene.scalePerHex=clamp104(fd.get('scalePerHex'),.01,100000);scene.scaleLabel=String(fd.get('scaleLabel')||'м').trim()||'м';scene.fogMode=['off','cover','objects'].includes(String(fd.get('fogMode')||''))?String(fd.get('fogMode')):'cover';scene.fogEnabled=scene.fogMode!=='off';scene.showInitiativeToPlayers=fd.get('showInitiativeToPlayers')==='on';scene.backgroundColor=String(fd.get('backgroundColor')||scene.backgroundColor);if(!local.store.categories.includes(scene.category))local.store.categories.push(scene.category);sortCombatScenes();queueStoreSave();Combat.render();broadcastScene104();}
