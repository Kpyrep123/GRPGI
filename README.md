...
В профиле ДМа при запуске через `npm start` доступна панель для повышения patch-версии, сборки NSIS и атомарной публикации desktop-релиза на `app.grpg-sync.ru`, прямого SSH-деплоя `deploy/site` и создания очищенного ZIP исходников. Установленная Electron-сборка эту панель не показывает. GitHub не участвует в выпуске и проверке обновлений.

Настройка: [`DEV_AUTOMATION.md`](DEV_AUTOMATION.md).
## Карты регионов и тактическое управление

Для ДМа доступен единый **Region Command Center**: карты планет, иерархия регионов/городов/зданий, флот, ракеты, РЛС/РЭБ, маршруты, туман войны и вывод на второй экран. Старые дублирующие редакторы перенаправлены в этот интерфейс.

Руководство: [`REGION_COMMAND_CENTER.md`](REGION_COMMAND_CENTER.md).


## Web-клиент 1.0.44

На desktop-экранах web-клиент использует интерактивную галактическую карту с системами, маршрутами, поиском и орбитальным просмотром планет. На телефонах автоматически используется облегчённый навигатор без карты.

Web-клиент сразу открывает вход за персонажа. URL PocketBase и Campaign ID задаются приложением; отдельный экран «Подключение к кампании» удалён. На новом устройстве техническая `app_users`-сессия создаётся в раскрывающемся блоке того же экрана входа.
