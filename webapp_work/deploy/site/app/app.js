(() => {
  ...
  const WEB_CLIENT = (() => {
    ...
    return !isNative && /^https?:$/i.test(window.location.protocol || '');
  })();
    ...
    root.innerHTML = `
      <div class="profile-card" style="padding:16px;">
        ...
        <div class="profile-toolbar">
          <button class="ghost-btn mini-btn" type="button" data-action="profile-refresh">${WEB_CLIENT ? 'Обновить из облака' : 'Обновить'}</button>
          <a class="ghost-btn mini-btn" href="https://app.grpg-sync.ru" target="_blank" rel="noopener">Загрузка</a>
  ...
  async function handleSetupSave(form) {
    ...
    openBoot('login');
    renderLogin();
    $('#setup-status').textContent = WEB_CLIENT ? 'Готово. Веб-версия не сохраняет конфигурацию и кеш в браузере.' : 'Готово.';
    notify(WEB_CLIENT ? 'Веб-клиент подключён к облаку' : 'Подключение сохранено', 'ok');
  }
  ...
  function bindGlobalEvents() {
    ...
    window.addEventListener('offline', () => notify(WEB_CLIENT ? 'Сеть пропала. Веб-версия не хранит локальный кеш.' : 'Сеть пропала, остаёмся на локальном кеше', 'warn'));
  }
  ...
  async function init() {
    if (WEB_CLIENT) document.body.classList.add('web-client-mode');
    bindGlobalEvents();
    await loadLocalState();
    fillSetupForm();
    if (WEB_CLIENT && $('#setup-status')) $('#setup-status').textContent = 'Веб-версия работает только через облако: конфигурация, кеш и профиль не сохраняются в браузере.';
    ...
    if (hasConfig()) {
      try {
        ...
        notify(WEB_CLIENT ? `Облако недоступно: ${error.message}` : `Загружен локальный кеш. Облако сейчас недоступно: ${error.message}`, 'warn');
      }
      renderLogin();
