...
function drawWebEraSystemMarkerV1050(ctx, p, r, palette, active = false, markerColor = '', markerStyle = 'orbital') {
  ...
  renderMarket=function(){
    ...
    const categoryPicker=marketTabWebV1073==='goods'?`<div class="market-categories-v144" role="group" aria-label="Категории товаров"><button class="secondary ${goodsCategoryWebV144==='all'?'active':''}" type="button" aria-pressed="${goodsCategoryWebV144==='all'}" data-web-market-category-v144="all">Все <span>${visibleOffers.length}</span></button>${categories.map(row=>`<button class="secondary ${goodsCategoryWebV144===row.label?'active':''}" type="button" aria-pressed="${goodsCategoryWebV144===row.label}" data-web-market-category-v144="${esc(row.label)}">${esc(row.label)} <span>${row.count}</span></button>`).join('')}</div>`:'';
